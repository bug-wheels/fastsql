...
public class CrudRepositoryImpl implements CrudRepository {
  ...
  public <T> T findById(T entity, String... columns) {
    try {
      ...
      if (columns == null || columns.length == 0) {
        ...
        sql =
          "select " + columnsName.toString().substring(0, columnsName.length() - 2) + "  from  " + tableName + " where "
            + where.toString();
      }
  ...
  @Override
  public <T> PageListContent<T> getEntityPageListBySql(String sql, Object[] args, T entity, int pageNum, int pageSize,
    String orderBy) {
    return null;
  }
